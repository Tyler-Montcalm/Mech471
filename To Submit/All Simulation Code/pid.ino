...
void PID(float wdesired){
 ...
 if (t < 20){ //Calculates the errors for the PID control. 
  ...
  if (errorpf < -50){
   ...
   if(errorpf * (wdesired-startspeed) > 0) errorpf *= 0.001 ; //This causes a quicker settling time. Effectively a larger control signal is needed to lower overshoot as the control has outpaced the actual
  ...
  //This is the tuned PID controller values, it encorporates PID for the backwheel, aswell as an addition proportional factor for the front wheel. 
   if(errorpb * (wdesired-startspeed) > 0) errorpb *= 0.001 ;
   controlinput = errorpf*0.0001+errordf*0+errorif*0.0001+errorpb*0.0005+errordb*0.0001+errorib*0.0001;
