...
char *p_smem; // BW: pointer to shared memory
...
int APIENTRY WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, int )
{
	...
	// BW create/access shared memory ////

	p_smem = shared_memory(name,n);
	
	if( !p_smem ) return FALSE; // error
...
HRESULT UpdateInputState( HWND hDlg )
{
	...
	// BW write joystick states to shared memory block
	...
	q[3] = (int)js.lRx;
	q[4] = (int)js.lRy;
	//q[5] = (int)js.lRz; Does nothing for XBox controller
